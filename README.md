<p align="center">
  <img src="images/rippled-field-guide-logo.png" alt="The Rippled Field Guide" width="400">
</p>

Operational intelligence for XRP Ledger validators and stock nodes. Compiled from production experience and core developer insights.

## Contents

- [The Rippled Field Guide](docs/rippled-field-guide.md)

## What's Inside

Battle-tested best practices for running rippled, including:

- **Database Management** - `online_delete` tuning and why higher values mean less I/O
- More topics coming...

## Philosophy

Each entry follows the same format:
1. **Bottom-line recommendation** - what to do
2. **Technical explanation** - why it works
3. **Configuration example** - how to implement
4. **Source** - where the insight came from

## Contributing

Found something that should be in here? Open an issue or PR.

## License

[MIT](LICENSE)

## Maintainer

[grapedrop.xyz](https://grapedrop.xyz)
